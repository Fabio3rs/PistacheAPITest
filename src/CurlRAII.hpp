...
public:
    ...
    /**
     * @brief CURL callback definition, if userp->storedata is false it will store nothing
    ...
    */
    static size_t cb(void* data, size_t size, size_t nmemb, curlCallBack* userp)
    {
        size_t realsize = size * nmemb;

        if (userp->storedata)
            userp->buffer.insert(userp->buffer.end(), reinterpret_cast<const char*>(data), reinterpret_cast<const char*>(data) + realsize);

        return realsize;
    }

    curlCallBack() : response_code(0), curl_perfm_res(CURLE_OK), storedata(false)
    {
...
public:
    ...
    */
    ...
    /**
     * @brief RAII version of curl_slist_append, create and append headers for cURL
     * @param[out] ptr The curlslitraii_t variable to create/append
    */
    static inline void curl_slist_append_raii(curlslitraii_t& ptr, const char* str)
    {
        ...
        /*
        cURL returns the first element from the curl_slist_append, that's what I am expecting in this function logic
        */
        if (ptr.get() != tmp)
        {
            throw std::logic_error("curl curl_slist_append return ptr different");
        }
    }
    ...
    */
    curlCallBack dispatch_github_action(const std::string& url, const std::string& GITHUBTOKEN, const std::string& payload);

    /**
     * @brief singleton of the CurlRAII class to call automatically curl_global_init
